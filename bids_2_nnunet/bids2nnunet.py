...
def bids2nnunet(bids_path, nnunet_path):
    ...
    for subdir, dirs, files in os.walk(bids_path):
        for file in files:
            ...
            if regexp_sub.search(file) and ext == ext_gz and "angio" in file and "derivatives" not in subdir:
                ...
                sub_nb = int(''.join(char for char in sub if char.isdigit()))
                ses = re.findall(r"ses-\d+", subdir)[0]
                print("{}) Copying files for {}_{}".format(cnt, sub, ses))
                mapping_bids2nnunet["{}_{}".format(sub, ses)] = "angio_{}".format(str(cnt).zfill(3))
                if sub_nb < 450:
                    patients_with_weak_labels.append("{}_{}".format(sub, ses))
    ...
    # save list to disk
    open_file = open("/home/newuser/Desktop/MICCAI_Aneurysms/patients_with_weak_labels.pkl", "wb")
    pickle.dump(patients_with_weak_labels, open_file)
    open_file.close()

    # save mapping dict to disk
    open_file = open("/home/newuser/Desktop/MICCAI_Aneurysms/mapping_bids2nnunet.pkl", "wb")
    pickle.dump(mapping_bids2nnunet, open_file)
    open_file.close()


def main():
    bids_ds_path = "/media/newuser/7EF9B56B27259371/Aneurysm_Data_Set/29-BIDS_Aneurysm_dataset_May_05_2021/"  # path to BIDS dataset folder
